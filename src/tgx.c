...
tgx_t *tgx_new(account_t *);
int tgx_run(tgx_t *);
int tgx_poll(tgx_t *, int timeout);
int tgx_wait(tgx_t *);
int tgx_exit(tgx_t *);
void tgx_free(tgx_t *);
...
// API
void tgx_api_check_auth_code(tgx_t *t, char *code);
void tgx_api_check_db_encryption_keys(tgx_t *t);
void tgx_api_get_chats(tgx_t *t, int64_t order, int64_t offset, int32_t limit);
void tgx_api_search_contacts(tgx_t *, const char *query, int32_t limit);
void tgx_api_send_message(tgx_t *, int64_t cid, const char *msg);
void tgx_api_set_auth_phone_number(tgx_t *t, char *number);
void tgx_api_set_parameters(tgx_t *t);
...
init_plugin_info(void)
{
	static struct plugin_info info = {
	    ...
	    "Guillaume Douézan-Grard <gdouezangrard@gmail.com>",
	    "https://github.com/gdouezangrard/bitlbee-telegram"};
